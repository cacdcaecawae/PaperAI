...
PaperAI 开放三个产品自有引擎——内置 DSH 循环、Codex 与 Claude——默认选择 Codex，并关闭用户 preset 根目录。已有用户自建 preset 文件仍保留在磁盘上，但不出现在 PaperAI 选择器中。Codex 与 Claude 的 ACP 工厂贡献各自的可选路由。ACP 设置页分别展示各渠道的实际连接、安装和握手观测状态。其他 DSH profile 保留随附及用户自建的 preset。
