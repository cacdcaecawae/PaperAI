...
PaperAI 工作台通过既有 `ui-layout` 服务配置 480–1280 px 的详情栏范围、860 px 的打开宽度（可完整显示一页 A4）、360 px 的中栏下限，以及 `current-session` 详情可见条件。
...
PaperAI 开放三个产品自有引擎——内置 DSH 循环、Codex 与 Claude——默认选择 Codex，并关闭用户 preset 根目录。已有用户自建 preset 文件仍保留在磁盘上，但不出现在 PaperAI 选择器中。Codex 与 Claude 的 ACP 工厂贡献各自的可选路由。ACP 设置页分别展示各渠道的实际连接、安装和握手观测状态。其他 DSH profile 保留随附及用户自建的 preset。 该档案还为原生引擎声明了一条阿里云百炼（DashScope 兼容模式）模型路由，因为 pi-ai 目录只带千问 Token 计划端点；key 在模型页以 `DASHSCOPE_API_KEY` 录入。
