...
The PaperAI workbench configures the existing `ui-layout` service with a 480–1280 px details range, an 860 px opening width that shows one A4 page at full size, a 360 px center floor, and `current-session` details eligibility.
...
PaperAI exposes its three product-owned engines — the built-in DSH loop, Codex, and Claude — defaults to Codex, and disables the user preset root. Existing user-authored presets remain on disk but do not appear in PaperAI's picker. The Codex and Claude ACP factories contribute their selectable routes. The ACP settings page shows each channel’s actual connection separately from installation and handshake observations. Other DSH profiles retain their shipped and user-authored presets. The profile also declares an 阿里云百炼 (DashScope compatible-mode) model route for the native engine, since the pi-ai catalog ships only the Qwen Token Plan endpoints; the key is entered on the Models page under `DASHSCOPE_API_KEY`.
