...
会话模型选择属于会话领域约定。`session.models` 将当前 `ModelSelection` 与按提供方分组的建议性模型、精确模型的推理元数据和逐提供方查询失败记录分开返回。该选择可能不在这些分组中，也绝不会作为合成行注入；客户端可以提示用户作出另一项选择，而无需把目录变成路由白名单。`session.selectModel` 校验由适配器持有的可选推理强度，并指定下次组装提示词时使用的完整选择。目录成员关系不构成校验：Agent 持有的模型控制器或 LLM 适配器都可以解析未列出的模型，而不可用的提供方或不受支持的推理强度会返回 `model-unavailable`。对于由 Agent 驱动持有的会话，`session.models` 会把驱动方通告的推理等级投影为同一种精确模型推理元数据，并把它的布尔开关（提供方的 fast 模式）列为 `switches`；上报的选择携带已应用的等级与每个开关的值，`session.selectModel` 则把可选的 `switches` 映射连同推理强度一起转交驱动方，驱动方会拒绝它未通告的等级或开关。`session.models` 还会报告 `routable`，即当前是否有 Agent 驱动或 LLM 适配器为所选提供方提供服务。该值刻意不从分组推导，因为两种属主都可以服务未公布的模型。`session.prompt` 会依据同一事实，在开启轮次之前以 `model-unavailable` 拒绝；客户端禁用 composer 只是提示性设计，这个方法始终可被调用。
...
`agentPreset.list` 领域向浏览器暴露部署的 preset 名单，使其在开启会话时能够提供选择；每一行携带它的 `trust`（`user` preset 的权限恰好等于它所引用的插件）、它是否为当前默认值，以及——当该 preset 无法组装会话时——一条 `broken` 原因：损坏的目录仍占着它的 id，界面必须能展示并删除它，而不是把它端出来然后在会话启动时失败。未组装任何 preset 的部署返回空名单而非错误，因为共用宿主组装本身就是一种有效部署。`agentPreset.select` 只允许把空白且空闲的会话切换到另一个 preset：目标位于当前工厂路由时重组存活 Agent 的作用域，目标位于另一个路由时先为 Session 建立检查点、停稳旧 Agent，再通过目标工厂恢复同一身份；替换失败会恢复日志记录的路由与组装。一旦跑过任何轮次，那段历史就是在原 preset 的工具下产生的，替换会留下无法执行的已记录工具调用，此时返回 `agent-preset-locked`。`codex` 与 `claude` preset 在新建、持久化恢复、fork 和冷恢复中都要求存在同名工厂路由；缺失会显式失败，绝不回退到默认 DSH loop。

模型列表、模型选择、提示词和预设替换共用按会话串行的准入队列。模型请求在先前的替换完成后解析 Agent，避免读取正在关闭的驱动或在替换间隙恢复旧驱动。图像准入仍在该会话操作内部与模型选择保持顺序。
