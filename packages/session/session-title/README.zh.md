...
只有用户 `user/message` 事件中的文本块符合条件。第一条符合条件的提示词会安排回退，从其首个非空行的开头若干词生成标题，并受所配置 UTF-8 字节上限约束。系统先移除终端控制序列再选择该行，规范化行内空白，且截断绝不会切断码点。后续行及其中展开的引用内容保留在原始消息中。空提示词和非文本提示词会等待后续符合条件的输入。
