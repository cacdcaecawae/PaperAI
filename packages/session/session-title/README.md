...
Only text blocks from human `user/message` events are eligible. The first eligible prompt schedules a fallback from the first words of its first non-empty line within the configured UTF-8 byte limit. Terminal controls are removed before choosing that line; its whitespace is normalized, and truncation never splits a code point. Later lines, including expanded reference content, remain in the original message. Empty and non-text prompts wait for later eligible input.
