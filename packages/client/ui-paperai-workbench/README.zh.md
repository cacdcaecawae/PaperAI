...
DSH 原生的 PaperAI 文档 UI 插件。它向 `sidebar.workspaces.content` 贡献扁平项目资源树，并向 `conversation.details.view` 贡献完整的 `paperai` 视图；不会替换 Workspace 浏览器、会话壳层、工具详情或 `ui-layout` 的几何规则。
...
Working DOCX 始终是唯一权威正文。右栏复用 DSH 的详情 host 结构：紧凑标题行、行式标签页、弱分隔、正常的 13–14px 字号以及行级操作。预览在严格只读沙箱中渲染 OfficeCLI 派生的 HTML；完整 HTML 不会进入任何编辑回调。编辑页展示 Host 投影的语义节点目录，并且只为当前选中的可编辑节点请求临时纯文本 buffer。用户选择“提交并创建版本”之前，buffer 仅存在于浏览器；提交时只发送一个由 `nodeId` 定位的 `replace-text` mutation。buffer 有未提交修改时会禁止切换节点、打开另一文档和恢复历史版本；加载外部新版本时，如果该节点未变化，controller 会把草稿更新到新基线，如果同一节点也有外部修改，则保留草稿并报告冲突。“放弃修改”只在本地恢复。版本列表展示持久化摘要以及人工或 Agent 来源，其中包括 Agent 客户端和精确模型；只有 Host 标记为可恢复的提交才出现恢复操作。模板门禁展示已选的内置或上传模板、最近检查结果以及一个有真实后端支撑的运行门禁操作。
