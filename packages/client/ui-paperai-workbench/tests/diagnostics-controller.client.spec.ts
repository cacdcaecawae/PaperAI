...
describe('diagnostic observations', () => {
  ...
  it('clears probe activity after protocol and transport failures and ignores failures after disposal', async () => {
    const remote = successfulRemote()
    const controller = new DiagnosticsController(remote)
    const diagnostic = await remote.probeAgent({ provider: 'codex', force: false })
    if (!diagnostic.ok) throw new Error('Missing diagnostic fixture')
    remote.agentDiagnostics = vi.fn().mockResolvedValue({ ok: true, value: [{ ...diagnostic.value, connected: true }] })
    await controller.loadAgents()
    controller.disconnected()
    expect(controller.store.getSnapshot().agents.every(agent => agent.connected === false)).toBe(true)
    await controller.loadAgents()
