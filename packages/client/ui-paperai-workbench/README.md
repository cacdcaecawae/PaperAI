...
The paragraph-style menu displays only the current document's defined style names and submits their stored IDs. “Apply style” is an action placeholder; only an explicit draft selection supplies its displayed value because the preview does not report each paragraph's style ID. An empty catalog disables the control. Deferred commit replies retain the previous catalog until the background preview refresh returns its replacement, including while another draft is open.
