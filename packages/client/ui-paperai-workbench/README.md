...
The command row exposes draft undo/redo, font, point size, bold/italic/underline, character-format clearing, a paragraph menu (style, alignment, left indent, line spacing), and find; every choice is a DSH menu, the pending bar holds the one save button, and a right-click on selected text offers 交给 Agent. Character readings distinguish inherited, explicit, and mixed values. A retained selection supports formatting across mapped blocks; a collapsed caret targets subsequent input. Enter splits ordinary body paragraphs, Shift+Enter inserts a soft break, and pasted line breaks become structural drafts. Composition input is grouped for undo. Drafts stay addressed by their original nodes, carrying paragraphs, runs, and layout to one transactional commit. Ctrl/Cmd+S or Ctrl/Cmd+Enter saves; Ctrl/Cmd+Z and redo affect local drafts, while Escape discards the focused block's draft.

Drafts retain complete rendered character runs for repaint and undo. Commits carry only properties whose rendered values changed from the initial preview, keeping fallback fonts and unchanged sizes out of text-only and bold-only edits. Cleared values remain explicit. Selecting an already displayed value is a no-op; initial readings stay local to the browser. The [format-intent decision](../../../.agents/notes/implemented/bug-fix/2026-09-13-paperai-rendered-format-intent.md) owns character alignment and metadata preservation rationale.

When Host rendering is deferred after saving text, the temporary preview represents each committed paragraph as a separate sibling block of the original semantic element type, retaining run formatting and paragraph layout. HTML serialization and reparsing keep those paragraphs independently matchable to the new node index. Empty paragraphs retain formatted spans without caret-placeholder breaks; real soft breaks remain. Restore and template changes clear the old page until the committed revision renders; a failed render offers a read retry without repeating the successful commit. A same-revision background render replaces the preview only when it has no new draft or active composition, including retained document views. Composition completes into one undoable draft before automatic external refresh is reconsidered; a completed draft keeps the external-update notice for an explicit refresh.

The paragraph-style submenu displays only the current document's defined style names, marks the current reading, and submits their stored IDs; a style outside the catalog appears as a disabled row. Only an explicit draft selection supplies a reading because the preview does not report each paragraph's style ID. An empty catalog disables the entry. Deferred commit replies retain the previous catalog until the background preview refresh returns its replacement, including while another draft is open.

Template, format-check, and version panels open one at a time beside the page, temporarily taking the conversation's space and restoring focus when closed; narrow columns stack the panel. Version comparison shows the compared version's own page with every change marked in place, removed paragraphs struck through where they stood; a second reading measures the current document from the picked version. Recorded format operations have a separate count, not a visual format diff. Comparison makes blocks read-only. Restoring requires confirmation and creates a new version; it never replaces draft undo. Export uses the saved document, is blocked by drafts, distinguishes unchecked draft output from formal delivery checks, and reports the output path or blocking findings.

The header's facts line reports the template, the format check, the version count, and the save state (in-memory drafts, saving, completion, and failed operations). A pill floating over the page zooms between fit-width and 50–200% while preserving relative scroll position, and its tooltip disclaims final Word pagination. A commit refused because the Working DOCX changed outside PaperAI offers to record that file as a version of its own and reopens the document with the draft kept; the project doctor offers the same for every document it flags. A second pill at the bottom right counts the rendered pages. Under the sidebar's document list, 大纲 lists the open document's headings, and a click scrolls the page to that heading.

`Config.retainedPreviews` is a positive integer, default `2`, including the active preview. Recent previews retain their DOM, scroll positions, and block drafts. Eviction releases the heavy preview while retaining lightweight drafts and scroll offsets until plugin disposal. Document changes invalidate inactive previews; reconnect re-reads loaded projections. Drafts conflict when their paragraph is deleted, becomes protected, or changes text or rendered character formatting. Unrelated external edits leave them writable. Conflicts retain the draft's complete runs until the writer chooses a side in the page. Drafts with no identifiable paragraph appear below the document, with copy and individual discard controls; discarding requires confirmation. Starting a document rejects an existing dirty draft. Drafting or navigation during import or template creation keeps the active view; the completed document is added to the project list.

Selecting mapped Word text floats a bar under the selection with “Ask Agent” and three canned requests (Polish, Expand, Check citations); a right-click offers the same list as a menu. Comparison pages remain selectable and copyable but omit Agent quotation actions because the diff has no version-specific node index. The gesture inserts a removable composer reference with document id, path, revision, head commit, block ids, and exact text; a canned request adds its fixed sentence after the reference, and the person still sends the message. Its serialized value freezes before asynchronous submission; changing documents cannot retarget it. The existing reference codec handles clipboard persistence, removal, and message serialization. The format-check panel's repair action appends its request after any existing composer text, preserving images and references, and reveals the conversation for review. These gestures never send a message.
