...
The plugin contributes the document list and Project Doctor, project start page, Templates settings page, and `paperai` document view. It retains DSH Workspace navigation, conversation, permission controls, and model selection. Registrations follow their declaring slots through `slots.inject()`. Colors and shared controls come from DSH. Workspace navigation and mounted views only read overviews; project initialization requires an explicit template choice or document creation through the [Host service](../../paperai/workbench-service/README.md).
...
When Host rendering is deferred after saving, the temporary preview represents each committed paragraph as a separate sibling block of the original semantic element type, retaining run formatting and paragraph layout. HTML serialization and reparsing keep those paragraphs independently matchable to the new node index. Empty paragraphs retain formatted spans without caret-placeholder breaks; real soft breaks remain. A same-revision background render replaces the preview only when it has no new draft, including retained document views.
