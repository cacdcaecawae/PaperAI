...
`Modal` 使用 `focus-trap` 将键盘焦点限制在对话框内，关闭时恢复仍连接在 DOM 中的触发控件且不滚动页面，并在最后一个对话框关闭前锁定 body 滚动。子编辑器可先消费 Escape，再由最上层对话框处理；已打开的子 `Menu` 会先消费 Escape 并恢复其触发控件。通过 portal 挂载到 body 的菜单仍按视口定位，并通过 DOM 归属标记加入所属对话框的焦点范围。菜单支持方向键、Home 和 End 导航，并跳过禁用项。对话框内容在视口内滚动，页脚操作仍可访问。调用方继续控制打开状态、是否允许关闭和本地化标签。
