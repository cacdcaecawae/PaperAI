...
`detailsPosition` 默认为 `end`，配置为 `start` 时 details 位于侧栏与对话之间，拖拽方向跟随可见边缘。主文档区打开时向对话 owner share 传递 `compact: true`。`ctx.layout.revealConversation()` 释放显式文档聚焦，两栏无法并排时优先显示对话，同时保留文档宽度偏好。再次打开文档会清除这一偏好，窗口变宽后可恢复并排。

可选 `Config` 字段 `centerMin`、`detailsMin`、`detailsDefault` 和 `detailsMax` 都是以 CSS 像素为单位的正整数。其默认值依次为 `640`、`300`、`360` 和 `520`；必须满足 `detailsMin <= detailsDefault <= detailsMax`，否则插件加载失败。详情栏会在保留 `centerMin` 的前提下向 `detailsMin` 收缩。拖动详情栏时会按配置范围限幅；重新打开时恢复偏好宽度，首次调整前使用 `detailsDefault`。
...
AppFrame 在稳定的 React 位置保留对话与 details。浏览器存储保留侧栏和详情栏偏好宽度及侧栏收起选择；当前详情栏是否打开、是否聚焦仍是临时状态。无效偏好回退到部署默认值。两个缩放分隔条支持左右方向键，配合 Shift 可按更大步长调整。零宽度的对话和详情区域不可交互，也不进入辅助技术导航。对话 owner 接收只影响布局的可选 `compact` 标记，侧栏接收 `collapsed` 与 `width`。业务数据仍来自标准 hook，操作由注册者拥有。
...
- **面板可见性是临时状态**：重新加载及切换到不同且符合条件的 Session id 都会关闭详情栏，同时保留偏好宽度，由产品决定何时重新打开。浏览器存储不可用时，偏好无法跨刷新保留。
