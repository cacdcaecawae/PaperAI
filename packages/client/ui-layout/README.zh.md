...
AppFrame 在稳定的 React 位置保留对话与 details。临时存储以默认侧栏宽度、关闭的 details 启动，不使用 `localStorage`。对话 owner 接收只影响布局的可选 `compact` 标记，侧栏接收 `collapsed` 与 `width`。业务数据仍来自标准 hook，操作由注册者拥有。
