...
AppFrame retains the conversation and details at stable React positions. Its transient store starts with the default sidebar width and closed details and never uses `localStorage`. The conversation owner receives the optional layout-only `compact` flag; the sidebar receives `collapsed` and `width`. Business data still comes from standard hooks and registrant-owned actions.
