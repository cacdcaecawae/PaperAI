...
`detailsPosition` is `end` by default or `start` to place details between the sidebar and conversation; drag direction follows the visible edge. An open primary details surface passes `compact: true` to the conversation owner share. `ctx.layout.revealConversation()` releases explicit details focus and prefers the conversation when both panels cannot fit, preserving the stored details width. Opening details again clears this preference, and widening restores a split when possible.

The optional `Config` fields `centerMin`, `detailsMin`, `detailsDefault`, and `detailsMax` are positive integer CSS-pixel values. Their defaults are `640`, `300`, `360`, and `520`; `detailsMin <= detailsDefault <= detailsMax` must hold or the plugin fails to load. Details shrinks toward `detailsMin` while preserving `centerMin`. Dragging details clamps to the configured range. Reopening restores the preferred width, or uses `detailsDefault` before the first resize.
...
`detailsNarrowMode` controls the result when an open details column cannot coexist with `centerMin` and `detailsMin`. The default `close` renders details at zero and gives the remaining width to center. `focus` keeps the current sidebar, makes the mounted center column inactive at zero width, and gives the remaining frame to details. The normal sidebar breakpoint, toggle, and resize behavior stays active in focus mode. Both modes preserve stored width preferences and recover the three-column split automatically when it fits again. Independently of the narrow fallback, `ctx.layout.setDetailsFocus(active)` lets a product demand the same focus presentation at any viewport width while details are open; closing the details panel releases the demand, and the width preference is never rewritten.

AppFrame retains the conversation and details at stable React positions. Browser storage preserves the preferred sidebar and details widths and sidebar collapse choice; current details visibility and focus remain transient. Invalid preferences fall back to deployment defaults. Both resize separators accept Left/Right arrows, with Shift for larger steps. Zero-width conversation and details surfaces are inert and hidden from assistive technology. The conversation owner receives the optional layout-only `compact` flag; the sidebar receives `collapsed` and `width`. Business data still comes from standard hooks and registrant-owned actions.
...
- **Panel visibility is transient** — reload and switching between distinct eligible Session ids close details while preserving preferred widths; products decide when to reopen it. Preferences cannot survive reload if browser storage is unavailable.
