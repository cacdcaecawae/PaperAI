...
空白 Session 的 Agent 初始化期间，chip 仍可选择并立即显示待切换值。每次只执行一个请求，后续选择合并为该 Session 的最新意图。失败时恢复最后成功的路由。作用域内的输入等待记录允许继续写草稿，同时阻止发送，直至替换完成。`conversation.hero.agentPreset.status` 是可选的根作用域 single 子 slot，接收 `presetId` 和 `connecting`，供产品独立展示缓存诊断，不替代实时模型选择。
