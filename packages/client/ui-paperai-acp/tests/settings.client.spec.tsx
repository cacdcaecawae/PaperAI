...
it('confirms destructive channel actions and validates provider headers without hiding the connection status', async () => {
  ...
  await act(async () => fireEvent.click(screen.getByRole('button', { name: /默认 Agent/ })))
  await act(async () => fireEvent.click(screen.getByRole('menuitem', { name: 'Claude' })))
  await click('一键检测')
  await click('刷新安装状态')
  ...
  await act(async () => fireEvent.click(screen.getByRole('button', { name: 'API 协议' })))
  await act(async () => fireEvent.click(screen.getByRole('menuitem', { name: 'anthropic' })))
