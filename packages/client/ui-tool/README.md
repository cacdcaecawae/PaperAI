...
Generic presenters select the displayed input, live progress text, and final output. Provider-owned names retain a nonblank generic presenter title in the summary when a call fails, alongside the failed state; complete error output remains available on expansion. Calls without that title keep the first error line as their failure summary. Durable result images use conversation's existing image slot beside the owning call, including results with a terminal or diff card.
