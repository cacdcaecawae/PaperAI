...
The plugin provides `ctx.paperMcp` and registers one exact Streamable HTTP route on `ctx.webServer`. `issueDescriptor(actor)` returns an ACP-compatible HTTP MCP descriptor and an idempotent disposer. The random Bearer token selects a lease-owned Agent identity, so callers cannot supply or rewrite mutation provenance through tool arguments.
...
The commit schema exposes only mutations implemented by the current commit service: replace text, insert a paragraph, delete a node, bind a template, and record a milestone. Before submitting `bind-template`, the handler requires an existing confirmed contract whose `appliesToRoles` includes the target document role; draft and incompatible contracts never reach `paperCommits.submit()`. A successful mutation returns both the complete `DocumentCommit` and its recorded `provenance`. Optimistic head and node-text conflicts retain the domain error code in the MCP error result.
