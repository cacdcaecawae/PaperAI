...
describe('HIT template-pack invariant companion', () => {
  it('reserves the package name through the invariant registry', async () => {
    ...
    expect(name).toBe('paperai-template-pack-hit-invariant')
    expect(inject).toEqual(['invariants'])
    await expect(apply(ctx)).resolves.toBe(dispose)
    expect(register).toHaveBeenCalledWith('@paperai/template-pack-hit', expect.any(Function))
