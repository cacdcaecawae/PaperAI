...
托管安装使用独立 npm 代次，并原子发布已验证的清单。取消保留当前安装；卸载只移除当前托管代次。旧代次保留给可能仍持有它的进程。SSH 要求显式配置 POSIX 主机、Node 和已安装的适配器。凭据通过 stdin 传输并严格检查主机密钥；会话拥有的反向隧道指向专用回环代理，只接受精确的 PaperAI MCP 路径及该 lease 的 Bearer 凭据。其他路径（包括本地 UI 和 `/api`）不会到达 Host 服务。运行时释放会关闭代理及其连接。远程会话不接收本地文件系统或终端回调。不支持 HTTP MCP 的本地适配器使用已有 MCP SDK 的 stdio 桥接。
