...
文字索引读取 OfficeCLI 的 JSON 记录，保留首尾空白、制表符及内嵌换行。正文段落和表格的数字地址按 OfficeCLI 视图规则计入第一层内容控件的子节点。每次替换、删除和相对锚点插入都把索引 `baseText` 与解析后的 XML 目标比较；不一致时，在发送任何写入命令前以 `NODE_TEXT_CONFLICT` 拒绝。段落检查使用编辑器的精确文字投影，表格检查使用索引中的行数标签。
...
- 不支持在块级内容控件内编辑。OfficeCLI 附加的文字（如自动列表编号）与可编辑 XML 文字不一致时，修改会被拒绝。
