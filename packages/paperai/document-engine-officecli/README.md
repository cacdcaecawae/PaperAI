...
Paragraph replacements inspect the OfficeCLI projection and the resident document's raw XML before writing. Namespace-aware XML parsing protects page and column breaks, fields, symbols, drawings, formulas, and other unprojected inline content. Unsupported run properties, including subscript, superscript, strikethrough, hidden text, and character-style references, also prevent replacement. Ordinary text, tabs, and soft line breaks are supported. A `paragraphs` replacement edits the original paragraph and inserts the remaining paragraphs immediately after it, using each insertion's returned Office path. New paragraphs inherit supported layout and character defaults, including font and size; explicit run overrides retain precedence. Paragraph format overrides cover style, alignment, indentation, and line spacing. Layout-only changes leave unchanged text and runs intact. OfficeCLI can normalize XML serialization and paragraph ids when saving; semantic document content, rather than ZIP entry byte identity, is the preservation criterion.

Run replacement also rejects different per-script fonts or sizes, independent complex-script bold or italic, theme-linked fonts and colors, and underline details beyond single or none. These values exceed the editable run fields, so replacing their text could change formatting that the writer did not select.

Soft breaks use vertical tabs in the document protocol. The paragraph setter creates the first run's breaks, and subsequent setters apply formatting without rewriting its text. Appended runs encode those breaks as line feeds for OfficeCLI, which writes Word break elements.
