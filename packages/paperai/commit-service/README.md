...
Service initialization attempts each retained journal independently, logging failures and retaining their journals while accepting work for other documents. Each FIFO operation retries its document's recovery before honoring caller cancellation; unresolved recovery rejects that operation. A journal whose head is still the recorded parent rolls back the Working DOCX and node index. A journal whose head is the recorded commit completes that commit and clears the journal. Recovery accepts only the recorded original or candidate Working SHA-256 and only node values from the journal's before/after states; an unknown head, file, node, snapshot, or conflicting commit raises `RECOVERY_FAILED` or the applicable snapshot failure and retains the journal without overwriting the unknown data.
...
DOCX snapshots are project-local content-addressed objects under `.paperai/objects/docx/<prefix>/<sha256>.docx`. Commit publication flushes snapshot and Working DOCX temporary file bytes before link or rename and sync parent directories on platforms that support directory fsync. Newly created snapshot directories are synced through the project root. Existing objects are verified before reuse; a corrupt regular object is atomically replaced only from bytes verified against its content address. Symlinks and non-regular objects are refused, and objects are never mutated in place. Node cannot fsync directories on Windows.
