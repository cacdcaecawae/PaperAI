...
Working DOCX 是唯一正文权威。预览 HTML 只用于显示，任何修改接口都不接受整份 HTML。编辑请求同时携带已观察到的文档 revision 与 head commit；成功后立即返回带人工来源的可恢复版本。段落提交在预览渲染之前就返回：浏览器先把已提交的文字写进当前预览，再在后台取回正式渲染的预览。段落拆分和格式通过同一个版本操作提交；先提交靠后的原始节点，避免插入操作使前面的原始位置地址失效。项目概览行包含权威的 `workingPath`，用于辨认同名文档和显示文件详情。

快照携带 `paragraphStyles`，原样传输文档引擎读取的样式 ID 和显示名称。延迟预览的段落提交返回空目录，由浏览器随预览刷新。目录读取失败也会返回空目录并记录原因，不会把已经成功的提交变成失败操作。
...
`diffVersion()` 通过文档引擎读取两份不可变快照，在段落级别解释一个版本相对其父版本的改动：相邻的删除与新增会配对为 `changed` 条目，根版本会把每一段都列为新增，`unchangedCount` 报告未改动的段落数。存在的 `formattingEditCount` 统计版本中已记录的、文字不变的格式操作数量；它不是完整的格式比较，尤其不能覆盖导入或恢复文件的全部格式变化。`open()`、`commit()`、`validate()` 与 `restore()` 维护文档投影本身；`restore()` 会从较早的版本创建新版本，而不是把 head 向后移动。响应还带有 `previewHtml`，即 Host 对该版本快照的渲染，浏览器据此显示该版本自己的页面，而不是把改动标在当前页面上。可选的 `baseCommitId` 让版本相对另一版而非父版本计量；响应中的 `steps` 携带完整的段落对齐，浏览器据此把每一处改动就地标出。
...
同一份生成的 Remote 提供 ACP 渠道目录、隔离检测与管理、托管安装、当前会话选项及外部历史关联和导入。这些方法把生命周期和校验委托给 [ACP 服务](../agent-acp/README.zh.md)；浏览器贡献共享这一挂载。
...
直接影响为零。项目概览、模板库列表、预览 HTML 与块级草稿保持为浏览器状态。
...
- 浏览器编辑定位原始语义节点，携带文字、run 与段落格式；复杂段内对象保持受保护。
