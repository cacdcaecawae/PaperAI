...
`exportDocument()` 返回判别结果。草稿发布和通过检查的正式交付返回带输出及里程碑版本工作台状态的 `status: 'success'`。因模板错误受阻的正式交付返回带未变 revision 与可投影门禁报告的 `status: 'blocked'`，既不创建输出，也不创建导出里程碑；其他导出失败仍会拒绝请求。
