...
`diffVersion()` explains one version against its parent at paragraph level by reading both immutable snapshots through the document engine: adjacent removals and additions pair up as `changed` entries, a root version lists every paragraph as added, and `unchangedCount` reports the untouched paragraphs. A present `formattingEditCount` counts recorded text-preserving formatting operations; it is not an exhaustive comparison of formatting, particularly for imported or restored files. `open()`, `commit()`, `validate()`, and `restore()` keep the document projection itself; `restore()` creates a new version from an earlier one instead of moving the head backward. The response also carries `previewHtml`, the Host's rendering of that version's snapshot, so the browser shows the version's own page instead of marking the current one. An optional `baseCommitId` measures the version from another one instead of its parent, and the response's `steps` carry the whole paragraph alignment so the browser can mark every change in place.
...
The same generated Remote exposes the ACP directory, isolated detection and management, managed installation, current session options, and external-history association/import. These methods delegate lifecycle and validation to [the ACP service](../agent-acp/README.md); browser contributors share this mount.
...
Zero direct tokens. Project overviews, template library listings, preview HTML, and block drafts remain browser state.
