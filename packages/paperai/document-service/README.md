...
Imports use private same-filesystem staging below `<project>/.paperai/documents/v1`. Published immutable originals live in `documents/source/` and authoritative Working DOCX files in `documents/working/`. The immutable source is copied to an exclusive final path, hashed against the staged bytes, and made read-only before its record is published. The Working DOCX is an independent writable file; editing either file cannot change the other. The document engine may keep the staged Working DOCX resident after indexing it; the service releases it before the staging directory is removed. Names already held by tracked documents remain reserved even if their files are missing; those conflicts and existing files receive ` (2)`, ` (3)`, and later suffixes. Existing records continue to use their recorded paths. Node identity is retained first by semantic hash, then related text and Office-path evidence; retained nodes keep lineage, style metadata, and last-commit attribution.
