...
Agent *创建* 由实现 `AgentFactory` 的插件提供。`setFactory` 安装默认驱动（`dsh-agent-loop`），`registerFactory(route, factory)` 则安装一个精确命名的同级驱动。这样，创建功能留在 `dsh-agent` 接口上，消费方（UI、ACP（Agent Client Protocol）桥接层）可以面向 `ctx.agents` 编程，而不依赖具体驱动包。注册表会把已经 traced 的 Service 规范化为具体目标，并通过调用方上下文重新 trace 每次调用；这既避免嵌套 Cordis shadow，也会把显式、绑定调用方的 `ownerCtx` 传给普通工厂。

- `ctx.agents.setFactory(factory: AgentFactory): () => void`：注册创建工厂（循环在构造时调用）。第二个工厂会导致抛出；dispose 时清空槽位。
- `ctx.agents.registerFactory(route, factory): () => void`：注册一个 effect-scoped 命名驱动。`hasFactoryRoute(route)` 报告精确可用性，`factoryRouteFor(agent)` 报告创建某个存活工厂 Agent 的精确路由（`undefined` 表示默认工厂）。缺失的命名路由绝不会回退到默认驱动。
- `ctx.agents.create(options: CreateAgentOptions): Promise<AgentHandle>`：创建会话和 agent，在不发布的情况下等待可选 setup，然后通过最终的 `SessionStore.enter()` 与 `AgentRegistry.enter()` 检查发布。不支持并发创建同一 ID：多个操作可以进行准备，但只有一个能进入；每个失败方都会回滚其私有作用域／会话／驱动器。可选且只用于创建的 `signal` 会取消未发布的 setup，并在返回 handle 前分离；之后的取消使用 `handle.dispose()` 或 `agent.cancel()`。发布包含在回滚范围内，回滚期间每条已交付创建边都会成对处理。`factoryRoute` 选择精确命名的驱动，省略时选择默认驱动；所选工厂缺失时拒绝。
- `ctx.agents.resume(options: ResumeAgentOptions): Promise<AgentHandle>`：加载持久化会话（[会话持久化](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.zh.md)），创建新的未发布 agent 作用域，等待可选 setup，并使用相同的最终进入发布序列。其可选 `signal` 与精确 `factoryRoute` 遵循创建规则。所选工厂缺失或未配置会话持久化时拒绝。
- `ctx.agents.recreate(options: RecreateAgentOptions): Promise<AgentHandle>`：通过另一个精确工厂路由恢复同一份持久化会话，从而替换一个空闲、由注册表工厂创建的 Agent。替换与回滚路由都会在 teardown 前验证；替换失败时恢复所提供的回滚组装。

`AgentHandle = { agent: Agent; dispose(): Promise<void> }`。Disposer 是一项 **消费方能力**；仅持有裸注册表条目的观察方不能 teardown agent。注册表只为显式 `recreate()` 操作保留其工厂 API 返回的 handle；调用方 fiber 与已注册工厂提供方仍是结构化共同拥有者。任意拥有者调用 `dispose()` 都会到达同一个记忆化完全停稳边界：它停止循环，等待循环退出，注销 agent，从存储中移除其会话，最后撤销其作用域世界。`ctx.agents.get(id)` 仍返回裸 `Agent`；ACP 桥接层与进程内 subagent 后端持有消费方 handle，而配置创建的 agent 已由循环 fiber 拥有。
