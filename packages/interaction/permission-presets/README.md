...
Two optional children ship the product surfaces over the same service: a `permissions` session-projection unit (`src/types.ts` declares the key; the unit folds the three whole-value knob events and views the select — table options plus a current-only `custom` — over the composition defaults) and the `/permission` command. A bare invocation reports the current preset and table. A named switch crosses the agent-scoped `permission/preset-apply` waterfall before any knob event is appended; listeners call `next()` only after provider-specific permission work accepts the target, and the owner rechecks the command signal at that final commit point. A rejection or cancellation fails the command and leaves the prior preset, sandbox mode, and approval policy durable. Provider exception text is logged rather than stored in the user-visible command record. Each child activates only when its registry (`ctx.sessionProjections` / `ctx.commands`) is composed.
