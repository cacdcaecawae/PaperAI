...
两个可选子功能在同一服务之上提供产品界面：`permissions` 会话投影单元（`src/types.ts` 声明该 key；单元以组合默认值为基础折叠三个全量值可调参数事件，并生成选择器视图，其中包含表内选项和仅作当前值的 `custom`）与 `/permission` 命令。不带参数调用时会报告当前预设与表。指定预设的切换会在追加任何调节项事件之前经过 agent 作用域的 `permission/preset-apply` waterfall；提供方特定的权限操作接受目标后，监听器才调用 `next()`，所有者会在该最终提交点再次检查命令信号。拒绝或取消会使命令失败，并保留此前持久化的 preset、沙箱模式和审批策略。提供方异常文本会写入日志，而不会存入用户可见的命令记录。每个子功能仅在其注册表（`ctx.sessionProjections` / `ctx.commands`）被组合时激活。
