...
describe('the shipped Web composition', () => {
  ...
  it('composes the full agent from `standard`', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent).filter(name => name !== 'glob' && name !== 'grep')).toEqual([
        'ask_user_question', NATIVE_SHELL_TOOL, 'create_goal', 'edit', 'exit_plan_mode',
  ...
  it('composes the exact RL prompt and two tools from `minimal`', async () => {
    ...
    try {
      ...
      expect(assembly.tools.map(tool => tool.name)).toEqual([NATIVE_SHELL_TOOL, 'str_replace_editor'])
      expect(assembly.tools.find(tool => tool.name === NATIVE_SHELL_TOOL)?.description).toBe(MINIMAL_SHELL_DESCRIPTION)
  ...
  it('keeps two differently composed sessions independent', async () => {
    ...
    try {
      expect(toolNames(ctx, minimal.agent)).toEqual([NATIVE_SHELL_TOOL, 'str_replace_editor'])
  ...
  it('composes the cordis agent with its own toolset', async () => {
    ...
    try {
      ...
      expect(tools).toEqual(expect.arrayContaining([NATIVE_SHELL_TOOL, 'read', 'edit', 'skill']))
  ...
  it('presents `code` as Code Mode without disturbing a native session beside it', async () => {
    ...
    try {
      ...
      expect(nativeAssembly.tools.map(tool => tool.name)).toContain(NATIVE_SHELL_TOOL)
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent)).toEqual([NATIVE_SHELL_TOOL, 'str_replace_editor'])
    } finally {
      await handle.dispose()
    }
  })
...
describe('a delegated child', () => {
  it('runs on the composition its parent runs on', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, child.agent)).toContain(NATIVE_SHELL_TOOL)
...
describe('authoring a preset on the shipped composition', () => {
  ...
  it('copies a shipped preset a session then really composes from', async () => {
    ...
    try {
      ...
      expect(toolNames(authorCtx, handle.agent)).toEqual([NATIVE_SHELL_TOOL, 'str_replace_editor'])
    } finally {
      await handle.dispose()
    }
  })
