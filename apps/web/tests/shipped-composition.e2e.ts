...
const BACKGROUND_COMMAND = process.platform === 'win32'
  ? "[Console]::Write('SHIPPED_BACKGROUND_OK')"
...
it('lets the native shell producer reach the background-job registry', async () => {
  ...
  try {
    ...
    // The platform shell is a preset row and `tasks` is a host registry; the producer
    ...
    const jobId = `${NATIVE_SHELL_TOOL}-1`
    const started = await ctx.tools.execute({
      signal,
      callId: CallId(`shipped-${NATIVE_SHELL_TOOL}-background`),
    ...
    expect(listed.content).toEqual([
      { type: 'text', text: expect.stringContaining(`${jobId} [${NATIVE_SHELL_TOOL}]`) as unknown as string },
    ])
