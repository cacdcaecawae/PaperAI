...
The enterprise keeps repo-restricted x64 larger-runner pools for Ubuntu and Windows. Pull requests in `deepseek-harness/deepseek-harness` run the three primary Linux jobs on the 16-core Ubuntu 24.04 pool and the independent native Windows signal on the 16-core Windows 2025 pool. Synchronized downstream repositories instead use standard GitHub-hosted runners, as the [downstream automation ownership decision](2026-09-01-paperai-downstream-automation-ownership.md) requires. The required Wine signal remains on standard hosted Linux. Public IPs are disabled, and workflow concurrency remains bounded because an autoscaling ceiling neither allocates idle machines nor makes repository work scale without limit.
...
The self-hosted serial Linux and Windows standby references and the disabled `serial-macos` job exist. Upstream DSH pull requests use the enterprise required path plus standard-hosted compatibility jobs, while other larger-runner sizes run only by manual dispatch.
...
The primary upstream topology pays one setup wave per 16-core lane and retains no workflow-level shard jobs or selectors. Process-local coverage partitions share that one setup and workspace. Every ordinary upstream DSH pull request consumes paid enterprise Linux and Windows minutes; manual benchmarks add other sizes only when remeasurement is useful.
...
Missing or renamed enterprise labels leave upstream DSH primary jobs queued. Standard-hosted compatibility jobs and `master` references still report useful evidence, but they do not substitute for the required aggregate; enterprise runner assignment therefore remains an upstream operational dependency. Downstream repositories avoid that dependency by selecting standard hosted labels before runner allocation.
