...
**Measure the gap as `dsh-v0.1.1-rc.2..upstream/master`, never by date.** Upstream lands between 49 and 326 commits a day, so a commit picked for sharing our import's date is already a thousand commits past the anchor and produces a diff six times too large. As measured on 2026-09-19 with the head pinned at `ddefc45fb` (0.1.6-alpha.2), GitHub's compare of `b150a551b8...ddefc45fb` reported 4912 commits. The same range is not directly countable here while the clone is shallow: `git rev-list --count b150a551b8..ddefc45fb` answers 18027, and adding `--ancestry-path` answers 4659. That option retains only commits in the range that descend from `b150a551b8`; it does not filter by merge-parent order and is not a substitute for the complete range. Quote the compare, and pin both ends when you do. Also on that date, `git merge-tree --write-tree --merge-base=b150a551b8 origin/main upstream/master` reported 292 conflicted paths with none under `packages/paperai/*`, `packages/client/ui-paperai-*` or `packages/bundle/paperai-web`.
