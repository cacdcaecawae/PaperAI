...
项目所有者另行授权分发每个官方 `@anthropic-ai/claude-agent-sdk` 版本，以及该版本通过 `optionalDependencies` 声明的官方 Claude Code CLI 与平台载荷。生成器将其表示为一项精确匹配直接包身份的例外，而非宽松许可证覆盖项：`SEE LICENSE IN README.md` 与 `SEE LICENSE IN LICENSE.md` 仍归类为非宽松，所有无关的非宽松运行时依赖仍以默认拒绝方式失败。存在该 SDK 时，生成器先选择交付型工作区包作为运行时依赖声明的唯一版本，再读取与该版本匹配的已安装 manifest；根目录链接到另一个传递版本时，不会改变生成的声明。随后生成器拒绝不符合官方 SDK 载荷前缀的可选包身份，推导 SDK、CLI 与载荷版本，核验已安装宿主载荷的身份、版本和声明许可证字段，并在单独的声明章节中渲染 SDK 声明的完整载荷集合。版本、声明许可证和载荷集合发生变化时无需新的身份授权，但仍须经过常规的依赖、锁文件、兼容性、条款和声明评审。
...
Claude 分发测试证明：只有精确匹配的直接 SDK 身份会绕过通常的非宽松运行时拒绝；该绕过不会改变许可证分类；载荷集合来自 SDK manifest，而非版本或平台允许列表。当 pnpm 安装多个 SDK 版本时，测试还把运行时版本选择钉在交付型工作区的声明上。SDK 身份错误、载荷缺失、存在无关的可选包身份、缺少运行时声明或多个交付型版本相互冲突时，测试都会失败。
