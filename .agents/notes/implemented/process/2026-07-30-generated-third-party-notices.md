...
The project owner separately authorizes distribution of every official `@anthropic-ai/claude-agent-sdk` version and the official Claude Code CLI/platform payloads that version declares through `optionalDependencies`. The generator represents this as one exact direct-package identity exception, not as a permissive-license override: `SEE LICENSE IN README.md` and `SEE LICENSE IN LICENSE.md` remain non-permissive classifications, and every unrelated non-permissive runtime still fails closed. When the SDK is present, the generator selects the sole version declared as a runtime dependency by shipping workspace packages, then reads that matching installed manifest; a repository-root link to another transitive version cannot change the generated notices. It rejects optional identities outside the official SDK payload prefix, derives the SDK, CLI, and payload versions, verifies the installed host payload's identity, version, and declared-license field, and renders the complete SDK-declared payload set in a separate notices section. Version, declared-license, and payload-set changes do not require new identity authorization, but they still require ordinary dependency, lockfile, compatibility, terms, and notices review.
...
The Claude distribution tests prove that only the exact direct SDK identity bypasses the ordinary non-permissive-runtime rejection, that the bypass does not change license classification, and that the payload set comes from the SDK manifest rather than a version or platform allowlist. They also pin runtime-version selection to shipping workspace declarations when pnpm installs more than one SDK version. Wrong SDK identities, missing payloads, unrelated optional package identities, missing runtime declarations, and conflicting shipping versions all fail.
