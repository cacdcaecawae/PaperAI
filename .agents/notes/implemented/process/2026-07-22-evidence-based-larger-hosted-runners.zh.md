...
企业保留仅限本仓库使用的 Ubuntu 和 Windows x64 大型运行器池。`deepseek-harness/deepseek-harness` 的拉取请求在 16 核 Ubuntu 24.04 池上运行 3 个 Linux 主作业，并在 16 核 Windows 2025 池上运行独立的原生 Windows 信号。同步下游仓库则按[下游自动化所有权决策](2026-09-01-paperai-downstream-automation-ownership.zh.md)使用 GitHub 标准托管运行器。必需的 Wine 信号仍位于标准托管 Linux。公网 IP 已禁用；工作流并发仍设有边界，因为自动扩缩容上限既不会分配闲置机器，也不意味着仓库工作可以无限扩展。
...
自托管的 Linux 与 Windows 串行热备参考，以及被禁用的 `serial-macos` 任务仍然存在。上游 DSH 拉取请求使用企业级运行器必需路径和标准托管兼容性作业，其他大型运行器规格仅通过手动触发运行。
...
上游主拓扑中的每个 16 核通道只承担 1 轮设置开销，且不保留工作流级分片 job 或选择器。进程本地 coverage 分区共享这 1 轮设置与同一个工作区。每个普通上游 DSH 拉取请求都会消耗付费的企业级 Linux 和 Windows runner 分钟数；只有在重新测量有价值时，手动基准测试才会加入其他规格。
...
企业级运行器标签缺失或改名时，上游 DSH 必需主作业会持续排队。标准托管兼容性作业与 `master` 参考流程仍会报告有用证据，但不能替代必需聚合流程；因此，企业级运行器分配仍是上游的运维依赖。下游仓库会在分配运行器前选择标准托管标签，从而避开这项依赖。
