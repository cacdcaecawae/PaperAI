...
Controller tests cover unchanged-node rebase, local, external, and merged same-node resolutions, commit requests against the latest base, a second same-node external head during merge, an unrelated-node head while the original conflict remains unresolved, cancellation of node reads, consumption of an older notification by a newer authoritative open, and preservation of a notification received in flight. Component tests expose both retained texts and all three localized actions while every conflicting document action remains disabled; opening the comparison focuses its heading, and internal Remote diagnostics are absent from rendered recovery messages. A keyless browser scenario imports a real Working DOCX, creates a same-node external commit followed by an unrelated-node commit through the Host, focuses and snapshots the still-present conflict surface, merges the texts, and commits successfully. Deferred workbench-service tests settle validation and export promises in both orders across successful export, blocked export, a new-revision validation, and an externally advanced head. Additional deferred commit, restore, and template-association tests start validation after the target revision is published but before the mutation Promise settles. The current report remains attached to its owning revision and the cache retains no more than one slot per document.
