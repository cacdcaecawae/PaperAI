...
PaperAI projects each DSH sandbox preset into a provider-supported native ACP permission mode. Codex maps `read-only`, `workspace-write`, and `danger-full-access` to `read-only`, `agent`, and `agent-full-access`; Claude maps them to `plan`, `acceptEdits`, and `bypassPermissions`. Codex receives the same target through `INITIAL_AGENT_MODE` when its process starts. New and loaded provider sessions must advertise the target and reconcile their current mode before the Agent is published. A user-initiated `/permission` switch traverses an awaited, Agent-scoped pre-commit waterfall: the ACP driver applies the target through `session/set_mode`, then delegates so the preset, sandbox, and approval events become durable. Provider rejection fails the command without changing the Session projection. The command record omits provider exception text and uses the client's localized fallback; the Host log retains the diagnostic. A switch during an active provider turn reserves maintenance before it revokes the process generation, cancels and drains the turn, and starts or synchronizes a replacement runtime with the target before delegation. The reservation suppresses automatic wake across the running-to-idle transition and replays a requested wake only after commit or failure, so queued input cannot start under the old native mode. Concurrent switches fail without releasing another switch's reservation. The waterfall owner checks the command signal at the durable commit point, preventing an aborted asynchronous listener from appending permission events. Direct `sandbox/mode` events outside the command remain serialized through the reconciliation observer, and the next prompt waits for the final selection. Each mode request combines the Agent lifecycle, process generation, and caller cancellation signals so a stalled provider request cannot continue to pin shutdown or a replacement prompt after cancellation revokes its generation. A missing advertised target is an explicit provider failure, not a reason to invent an identifier or retain a mismatched mode.
...
The PaperAI bundle test rejects product-owned `sandbox-policy` and `approval` rows. The built CLI config-dump test composes the real `paperai` profile and snapshots both effective rows as DSH-base contributions with `workspace-write` and `ask` defaults. Keyless Web snapshots boot the PaperAI overlay with the fake process only at the external ACP boundary, capture the fresh Workspace Write control, exercise a real switch to Read Only, and reject `session/set_mode` through the shipped Codex preset. They require the old projection, localized Toast and command row, and absence of provider diagnostics. ACP integration tests traverse the runtime protocol adapter for both providers and cover new, loaded, command rejection before durable permission events, active-turn command settlement, a queued follow-up held until the new mode commits, live-switch, prompt-ordering, restart, stalled-mode cancellation, and unadvertised-mode paths. The active-turn tests prove the old generation is cancelled and its terminal tool update is projected before the replacement process starts in Read Only, the permission events commit, and queued input is prompted. A deferred permission-presets test aborts after a listener starts but before it delegates and proves no permission event is appended. The same suite uses disjoint fallback and Session roots outside the platform temporary area: in-Workspace writes succeed, fallback-root and read-only writes fail at the final filesystem operation, and an already-dispatched write receives the revoked generation signal and cannot publish while the replacement turn resumes the ACP session in a fresh process.
