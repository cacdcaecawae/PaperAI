...
Preview retention has a configurable bound including the active view. Eviction releases heavy rendering while lightweight scroll offsets and drafts survive navigation. External changes invalidate cached previews. A changed block retains its local draft for copying or discarding and forbids overwriting the replacement block. Import and template creation refresh the project inventory but cannot replace a draft or navigation made while the request was pending: completing a remote operation does not grant ownership of later user input. DOCX commits remain the only durable edit path.
