...
文档变更通知会刷新已加载的项目列表，包括 Agent 在其他会话中新建的文档。再次选择同一块、尝试切换文档或连接恢复时，未保存的块文字都会保留。新建其他文档之前必须先保存或取消已有草稿。重连会读取当前版本而不替换草稿；遗漏的新版本通过外部更新横幅提示，读取期间收到的更新通知仍保留为待处理状态。项目概览分别携带所选 `templatePackId` 与可用套装，因此删除套装会显示模板缺失，不会把项目的选择改成自由写作。首次读取项目失败后，可以直接从起始页重试。
...
上游补丁集新增两处增量接缝——`conversation.hero.content` 插槽与 `LocaleRuntime.override`，与更早的 `setDraft` 和 `setDetailsFocus` 并列；由于 DSH 从来没有过其中任何一项，后续的 DSH 更新是重新应用这四项，而不是保留它们。完整清单由[视觉语法决策](2026-09-15-paperai-workbench-visual-grammar.zh.md)维护。客户端把 Host 预览视为不可信标记，因此依赖脚本或外部资源的预览会在没有它们的情况下渲染。块级编辑只触及带有提供方地址且文本匹配可编辑节点的块；未被索引的页眉、页脚、表格单元格、图和图题在浏览器中保持只读。删除自定义套装后，选用它的项目会显示模板缺失，直到重新选择，而已安装的格式继续可用。本文取代[多智能体论文写作基线](2026-09-02-multi-agent-thesis-writing-baseline.zh.md)中描述的侧边栏新建流程、带状态栏的标签页工作台和主题层；该记录的其他决策（自由模式、写作规程、门禁摘要、原生工具、访问范围）仍然有效，而其中"hero 文案没有覆盖 seam"这一后果已不再成立。
