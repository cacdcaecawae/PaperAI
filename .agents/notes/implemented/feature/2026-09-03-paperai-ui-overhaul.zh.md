...
上游补丁集新增两个可叠加 seam：`conversation.hero.content` slot 和 `LocaleRuntime.override`，与此前的 `setDraft`、`setDetailsFocus` 并列；之后合并 DSH 时必须保留这四处。客户端把 Host 预览视为不可信标记，因此依赖脚本或外部资源的预览会在没有它们的情况下渲染。块级编辑只触及带有提供方地址且文本匹配可编辑节点的块；未被索引的页眉、页脚、表格单元格、图和图题在浏览器中保持只读。删除自定义套装后，选用它的项目会显示模板缺失，直到重新选择，而已安装的格式继续可用。本文取代[多智能体论文写作基线](2026-09-02-multi-agent-thesis-writing-baseline.zh.md)中描述的侧边栏新建流程、带状态栏的标签页工作台和主题层；该记录的其他决策（自由模式、写作规程、门禁摘要、原生工具、访问范围）仍然有效，而其中"hero 文案没有覆盖 seam"这一后果已不再成立。
