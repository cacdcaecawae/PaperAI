...
**One additive driver seam, PaperAI's own.** `AgentDriverModel` and `AgentModelController` sit in `@deepseek-ai/dsh-agent` but exist in neither the [baseline](../process/2026-08-28-paperai-dsh-baseline.md) nor upstream, so a later DSH update re-applies this seam rather than keeping it. PaperAI extends `AgentDriverModel` with optional `reasoning` (driver-owned efforts plus the effort the driver applies by default) and `AgentModelController` with optional `currentReasoningEffort`, `switches` (driver-owned boolean switches with the driver's own names and descriptions), and a second `selectModel(model, { reasoningEffort?, switches? })` argument. A driver that advertises nothing keeps its previous contract; the DSH Agent Loop still routes through `ctx.llm` and exposes none of it.
