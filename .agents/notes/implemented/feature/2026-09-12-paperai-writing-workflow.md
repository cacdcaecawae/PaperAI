...
**Conflict every draft on any newer head.** A revision check catches a changed font or layout that text equality cannot see. It also conflicts every draft in the document each time an Agent commits one unrelated paragraph, which is most of what an Agent does, so the rule was replaced by per-block text and rendered character-format comparisons.
