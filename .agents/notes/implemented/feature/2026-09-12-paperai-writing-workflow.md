...
Partially superseded: the [visual-grammar decision](2026-09-15-paperai-workbench-visual-grammar.md) owns the toolbar's presentation (one row of menus) and the save state's place in the details header; the status bar it replaced is gone. The decisions below otherwise stand.
...
Working DOCX remains the source of truth. Browser drafts attach to original indexed nodes and may carry replacement paragraphs, character runs, and paragraph layout. The commit service validates the base revision and publishes through its candidate/version transaction. The [Word edit preservation decision](../bug-fix/2026-09-13-paperai-word-edit-preservation.md) owns original-node binding, ordered engine writes, metadata retention, and protected inline objects. The commit path reindexes the saved DOCX.

The persistent toolbar operates on a retained caret or selection and supports local draft history. Character readings distinguish mixed, inherited, and explicit formatting. Enter, soft breaks, plain-text paste, and composition input use the same draft path. A draft conflicts when the text of its own block moved, so one outside commit elsewhere leaves every unrelated draft alone; an outside change to that block’s formatting alone needs no conflict either, because the preview repaints such a draft from the block’s new rendering. A conflict is resolved in the page, by the [conflict-resolution decision](2026-09-21-paperai-conflict-resolution-in-the-page.md). History restoration is a separately confirmed new version. In-memory drafts are explicitly described as lost on reload.

Documents default to writing mode; collaboration is an explicit preference, also entered by submitting a Word reference to the composer. The existing layout service persists preferred widths and sidebar collapse, while transient panel visibility stays unpersisted. A Session-scoped workbench store persists writing and zoom preferences. Auxiliary panels open one at a time and restore focus; narrow panels stack beside the reading flow. Templates open on request, destructive template actions explain their impact, exports wait for saved drafts, and the save state stays separate from other failed operations. Version comparison counts recorded format-only operations without claiming a detailed format diff.
...
This partially supersedes the [UI overhaul](2026-09-03-paperai-ui-overhaul.md) and [minimal workbench](2026-09-09-paperai-minimal-workbench-and-outside-edits.md) decisions for template prompting, editor commands, focus defaults, and panel behavior, and the [character-formatting](2026-09-10-paperai-character-formatting.md) decision for toolbar placement and supported run properties. Their plugin ownership, template model, resident-engine lifecycle, and version publication decisions remain active; run inheritance follows the Word edit preservation decision. The [session-title decision](2026-07-21-log-backed-session-titles.md) retains its ownership and timing rules; the fallback now selects one visible line before applying its word and byte limits.
...
**Conflict every draft on any newer head.** A revision check catches a changed font or layout that text equality cannot see. It also conflicts every draft in the document each time an Agent commits one unrelated paragraph, which is most of what an Agent does, so the rule was replaced by a per-block text comparison and a repaint that carries a draft onto its block’s new rendering.
...
Component and controller tests exercise panel persistence, keyboard resizing, modal focus, portaled menus, driver changes, retries, structural input, composition, draft history, formatting, conflicts, and version actions. Host tests cover transaction validation and operation counts. The [Word edit preservation tests](../bug-fix/2026-09-13-paperai-word-edit-preservation.md#word-edit-tests) cover native template and MCP readbacks. Assembled PaperAI browser scenarios cover navigation, permissions, editing, checking, versions, export, and viewport/theme/locale layouts, including an empty model directory during a delayed Agent switch.
...
Browser formatting covers the represented character values while the engine retains other Word metadata. Editor commands collect every changed block before publishing synchronous store updates, so subscribers cannot repaint an uncollected block. Quoting a selection or preparing a format fix appends to the existing composer draft and preserves attachments.
