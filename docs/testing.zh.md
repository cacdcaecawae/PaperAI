...
- **覆盖率门禁**（`pnpm run test:coverage`）：DSH 上游门禁级运行，对 `packages/*/*/src` 按文件 100% 覆盖。未覆盖的行往往是门禁正确标记出的死代码（应删除），而非需要补写的测试。行覆盖率是必要条件，但永远不是充分条件：它证明行被执行过，不证明功能按交付预期工作。`packages/shell/pwsh-local/src` 的按文件 100% 覆盖需要真实的 `pwsh`：缺少它时其执行器套件会自动跳过，`vitest.config.ts` 会豁免该文件以使无 pwsh 的主机保持绿色，而 CI runner 自带 pwsh，仍按完整标准执行门禁。
- **真实 API e2e**（`pnpm run test:e2e`）：各提供方的真实测试由自己的密钥控制，缺少密钥时自动跳过，使 keyless CI 保持绿色（[Agent Note](../.agents/notes/implemented/testing/2026-06-19-real-api-e2e-ci.zh.md)）。托管 DeepSeek CI 默认用于上游；下游仓库配置 `DEEPSEEK_API_KEY_EXTERNAL` 后，以 `DSH_REAL_API_E2E_ENABLED=true` 显式启用。
